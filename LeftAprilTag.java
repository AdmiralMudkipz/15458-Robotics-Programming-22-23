...
public class LeftAprilTag extends LinearOpMode {
    ...
    public static boolean umkehren = true;
    ...
    public void doAuto(int a){
      ...
      while (opModeIsActive() && exit) {
        if (a == 3) {
          ...
          fahrrichtung(0, 0.5, 1.9);//forward .5p, 1.8s
          ...
          fahrrichtung(3,0.5,1);//left, pushes into corner .5p, 1s
          fahrrichtung(2, 0.5, 2.1);//right .5p, 2.2s
          fahrrichtung(0, 0.5, 1.9);//forward .5p, 1.8s
          fahrrichtung(1,0.5,0.05);//brake .5p, .05s
          fahrrichtung(3, 0.5, 1.2);//left .5p, 1.2 seconds
           endlich(true);
           exit = false;
        } else{//12.97V
          ...
          //direction, power, time, 0 = forward, 1 = back, 2 = left, 3 = right
          fahrrichtung(3,0.5,1);//right, pushes into corner .5p, 1s
          fahrrichtung(2, 0.5, 2.1);//left .5p, 2.2s
          fahrrichtung(0, 0.5, 1.85);//forward .5p, 1.8s
          fahrrichtung(1,0.5,0.05);//brake .5p, .05s
          fahrrichtung(3,0.5,2.25);//left .5p 2s
          /*drehen(true, 0.5, 0.35);
          armKontrolle(true, 1, .35);
          offen();
          eTime.reset();
          while(eTime.seconds() < 1){}
          eTime.reset();
          fahrrichtung(0, 0.5, 1.75);//forward (to the left)
          schliessen();  */        // fahrrichtung(3,0.5,1);//right, pushes into corner .5p, 1s
          // fahrrichtung(2, 0.5, 2.075);//left .5p, 2.2s
          // fahrrichtung(0, 0.5, 1.85);//forward .5p, 1.8s
          // fahrrichtung(1,0.5,0.05);//brake .5p, .05s
          //drehen(true, 0.5, 5);
          // armKontrolle(true, 1, .4);
          // offen();
          // fahrrichtung(0, 0.5, 1.8);//forward (to the left)
          // schliessen();
          // armKontrolle(true, 1, 0.4);
          // fahrrichtung(1,0.5,1);
          // drehen(false,0.5,0.6);
          // fahrrichtung(3,0.5,0.25);
          // armKontrolle(true,1,3);
    ...
    public void fahrrichtung(int i, double power, double zeit){
      ...
      arm.setPower(0.01);}
      public void drehen(boolean richtung, double power, double zeit){//3.05 s at .5 power is 360
        eTime.reset();
        if(umkehren){
          power *= -1;
        }
        while(eTime.seconds() < zeit){
          if(richtung){
            flMotor.setPower(-power);
            frMotor.setPower(power);
            blMotor.setPower(-power);
